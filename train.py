...
def parse_opt(known=False):
    ...
    batch = 1
    workers = 1
    last_dir = ROOT / 'runs/train'
    dir_list = glob.glob(str(last_dir)+"/*")
    
    last_pt = ROOT / 'yolov5x.pt' # init_model
    if dir_list:
        path = '/'.join(dir_list[-1].split("/")[-3:])
        last_pt = ROOT / os.path.join(path, 'weights/last.pt')
    print("[Save Path] : ", last_pt)
    
    parser = argparse.ArgumentParser()
    parser.add_argument('--weights', type=str, default=last_pt, help='initial weights path')
    ...
    parser.add_argument('--name', default='cd', help='save to project/name')
